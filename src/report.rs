...
use gtk::{glib, CompositeTemplate, CheckButton, Box, Button, Entry, Label};
use glib::{Binding, Properties};
use glib::subclass::Signal;
use std::process::{Command, Stdio};
use std::fs;
use chrono::Utc;
use crate::github::{CONFIG, load_config, create_github_issue};
...
mod imp {
    ...
    #[properties(wrapper_type = super::Report)]
    #[template(resource = "/org/gnome/controlpanelgui/ui/report.ui")]
    pub struct Report {
        #[template_child]
    ...
    impl ObjectSubclass for Report {
        const NAME: &'static str = "Report";
        type Type = super::Report;
    ...
    impl Report {
        ...
        fn on_a1_toggled(&self, button: &CheckButton) {
            if button.is_active() {
                ...
                match label.as_deref() {
                    None => self.obj().set_property("issue", self.entry_1.text().to_string()),
        ...
        fn on_a2_toggled(&self, button: &CheckButton) {
            if button.is_active() {
                let label = button.label(); 
                match label.as_deref() {
                    Some("App") => (self.qbox_3.set_visible(true), self.obj().set_property("related", String::from("App"))), // Show the application list
                    Some(text) => (self.obj().set_property("related", text.to_string()), ()),
                    _ => (self.qbox_3.set_visible(false), ()), // Hide the application list
                };
            }
        }
        #[template_callback]
        ...
        fn on_submit(&self, button: &Button) {
            ...
            let mac = match self.get_mac_address(mac_address_path) {
                Some(mac) => mac,
                None => { eprintln!("Can't get MAC Address"); String::from("") },
            };
            let sw = match self.get_sw_version() {
                Some(sw) => sw,
                None => { eprintln!("Can't get SW Version"); String::from("") },
            };
            ...
            if app.is_empty() {
                ...
                eprintln!("App is not selected {}", enable);
            } else {
            ...
            if enable {
                ...
                match rt.block_on(create_github_issue(&email_body)) {
                    Ok(_) => {            
                        ...
                        self.summary.set_label(summary_text.as_str());
                        self.summary.set_widget_name("success-text");
                        self.summary.set_visible(true);
                    }
                    Err(e) => {
                        self.summary.set_label("Error when sending report");
                        self.summary.set_widget_name("required-text");
                        self.summary.set_visible(true);
                        return;
                    }
                }
    ...
    impl WidgetImpl for Report {}
    impl BoxImpl for Report {}
}

glib::wrapper! {
pub struct Report(ObjectSubclass<imp::Report>)
